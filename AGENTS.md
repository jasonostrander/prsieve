...
First-time setup: copy `llm_config.example.json` to `llm_config.json` and fill in the endpoint/token/model. The file is gitignored. The default endpoint is the project's Instacart AI gateway (`https://aigateway.instacart.tools/proxy/prsieve/openai/v1`), which authenticates requests at the gateway itself — so **`token` is left empty** for it (an empty token is correct, not a misconfiguration). Only set `token` when pointing at a provider that requires a bearer key (e.g. OpenAI at `https://api.openai.com/v1`). `run.sh`/`release.sh` will warn and fall back to the example if `llm_config.json` is missing.
