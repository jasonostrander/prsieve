...
1. Generates the changelog entry **before** building (so a bad summary or an
   abort costs nothing). `scripts/generate_changelog.sh` feeds the commit
   subjects since the previous tag to the `claude` CLI, which writes a few
   brief, user-facing bullets. It prints them and — when publishing
   interactively — lets you press Enter to accept or `e` to edit them in
   `$EDITOR`. See [The changelog](#the-changelog) below.
2. Bumps `CFBundleShortVersionString` and `CFBundleVersion` in the bundled
   `Info.plist` (the source file stays at `0.0.0`).
3. Builds in release mode, copies `Sparkle.framework` into the bundle, and
   Developer ID-signs everything with hardened runtime + secure timestamp.
4. Zips the signed `.app` (via `ditto -c -k --sequesterRsrc --keepParent`).
5. Notarizes the zip via the `PRSieve-notarize` keychain profile, staples the
   ticket onto the `.app`, then re-zips so the archive contains the stapled
   bundle.
6. EdDSA-signs the zip using the Keychain-stored private key.
7. Prepends the new section to `CHANGELOG.md` and regenerates `appcast.xml`
   with the new version, signature, length, download URL, and the changelog
   bullets as HTML release notes (`<description>`).
8. Unless `--no-publish` is passed: commits `appcast.xml` + `CHANGELOG.md`,
   tags `vX.Y.Z`, pushes both to `origin`, and creates a GitHub Release with
   the zip attached and the changelog bullets as the release body.
...
`CHANGELOG.md` (newest entry first) is the source of truth for "what's new". On
each release `release.sh` prepends a `## X.Y.Z — YYYY-MM-DD` section whose
bullets come from the release agent, and reuses those same bullets in two
places:

- **In-app**, as the `<description>` of the appcast item — Sparkle's standard
  updater renders it as the release notes in the **Check for Updates…** dialog.
- **On GitHub**, as the body of the GitHub Release.

The summary is generated by `scripts/generate_changelog.sh`, which you can run
standalone to preview it:

```bash
./scripts/generate_changelog.sh              # changes since the latest tag
./scripts/generate_changelog.sh v0.9.2       # changes since a specific tag
```

It calls the `claude` CLI with the commit subjects since the previous tag and a
prompt asking for at most four brief, user-facing bullets (internal/build/CI
churn excluded). If `claude` isn't installed or errors, it falls back to the
cleaned-up commit subjects so a release is never blocked on the agent.

To override the summary entirely, export your own markdown bullets before
releasing:

```bash
CHANGELOG_NOTES=$'- Fixed the thing\n- Added the other thing' ./release.sh 1.2.3
```

When publishing interactively you can also press `e` at the prompt to edit the
generated bullets in `$EDITOR` before the build runs. To tune the wording the
agent produces, edit the prompt in `scripts/generate_changelog.sh`.
